    ...
    */
    ...
    sendTransport.on('connect', async ({ dtlsParameters }, callback, errback) => {
      try {
        ...
        await new Promise((resolve) => {
          socket.emit('connect-transport', { roomId, transportId: sendTransport.id, dtlsParameters }, resolve);
        });
        callback();
      } catch (error) {
        errback(error);
    ...
    sendTransport.on('produce', async ({ kind, rtpParameters, appData }, callback, errback) => {
      try {
        ...
        const { id ,otherProducerIds} = await new Promise<{id:string, otherProducerIds: string[]}>((resolve) => {
          socket.emit('produce', { transportId: sendTransport.id, kind, rtpParameters, roomId }, resolve);
        });
  ...
  async function consume(producerId: string) {
    ...
    socket.emit('create-receive-transport', { roomId }, async (data: { transportOptions: any; }) => {
      ...
      recvTransport.on('connect', async ({ dtlsParameters }, callback, errback) => {
        try {
          await new Promise((resolve) => {
            socket.emit('connect-receive-transport', { transportId: recvTransport.id, dtlsParameters, roomId }, resolve);
          });
          callback();
        } catch (error) {
          errback(error);
      ...
      const consumerParameters = await new Promise((resolve) => {
        socket.emit('consume', { producerId, transportId: recvTransport.id, roomId }, (consumerParameters: {
          consumerParameters: mediasoupClient.types.ConsumerOptions;
        }) => {
          resolve(consumerParameters);
        });
      });

      const consumer = await recvTransport.consume(consumerParameters as mediasoupClient.types.ConsumerOptions);
